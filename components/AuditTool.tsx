...
import { geminiService, AuditInputs } from '../services/geminiService';
import { mailerliteService } from '../services/mailerliteService';
import { AuditResult, SectionId, TechnicalCheckPoint } from '../types';
...
const MOCK_DEMO_RESULT: AuditResult = {
  ...
  aiBreakdown: [
    { name: 'ChatGPT-4o', visibilityScore: 18, recommendationLikelihood: 8, brandAuthority: 32, citationCount: 3, sentiment: 'Neutral', keyTakeaway: 'Recognized as a brand but rarely selected as a top recommendation.' },
    { name: 'Gemini 1.5 Pro', visibilityScore: 12, recommendationLikelihood: 4, brandAuthority: 22, citationCount: 1, sentiment: 'Neutral', keyTakeaway: 'Heavily reliant on Google Search data which limits proactive discovery.' },
    { name: 'Perplexity', visibilityScore: 45, recommendationLikelihood: 28, brandAuthority: 52, citationCount: 10, sentiment: 'Positive', keyTakeaway: 'Better indexing here due to strong blog content structure.' }
  ],
  pillarAnalysis: [
  ...
  traditionalRankings: [],
  competitors: [
    { name: "Top Industry Rival", visibilityScore: 88, primaryStrength: "High Search Volume", threatLevel: "High" }
  ],
...
const AuditTool: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const keys: (keyof AuditInputs)[] = ['brandName', 'industry', 'websiteUrl', 'keywords', 'location', 'serviceCategories'];
      keys.forEach(key => {
  ...
  const nextFormStep = () => {
    setError('');
    const fields = ['brandName', 'websiteUrl', 'industry', 'location', 'serviceCategories', 'keywords'];
    if (!(formData as any)[fields[formStep]]) { setError('This field is required'); return; }
    if (formStep < 5) setFormStep(prev => prev + 1);
    else setStep('SCANNING');
  };

  const prevFormStep = () => { setError(''); if (formStep > 0) setFormStep(prev => prev - 1); };
  const handleKeyDown = (e: React.KeyboardEvent) => { if (e.key === 'Enter') { e.preventDefault(); nextFormStep(); } };

  // Logic to run analysis
  useEffect(() => {
    if (step === 'SCANNING' && !result) {
      const runAnalysis = async () => {
        try {
          const data = await geminiService.analyzeBrand(formData);
          setResult(data);
          // We do NOT set step to COMPLETE here. We wait for the progress bar to finish.
        } catch (e) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      // Send inputs + link to MailerLite
      ...
      // Since result is already computed during SCANNING, just show it
      setStep('COMPLETE');
      setActivePage('COVER');
    } catch (err) {
      console.error("Signup error", err);
      // Even if email fails, show the report to the user
      setStep('COMPLETE');
      setActivePage('COVER');
    }
  };

  const radarData = (result?.pillarAnalysis && result.pillarAnalysis.length > 0) 
  ...
  const navItems: { id: ReportPage; label: string; icon: string }[] = [
    { id: 'COVER', label: 'Cover Page', icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253' },
  ...
  return (
    <div className="w-full p-0.5 bg-gradient-to-r from-lime-400 via-emerald-400 to-lime-400 shadow-[0_0_120px_-20px_rgba(217,255,0,0.3)] rounded-[7px]">
      <div className="w-full bg-slate-950 border border-white/5 min-h-[600px] flex flex-col relative overflow-hidden rounded-[7px]">
        {step === 'INPUT' && (
          <div className="max-w-xl mx-auto w-full p-16 animate-in fade-in duration-500 relative z-10">
            ...
            <div key={formStep} className="animate-in fade-in slide-in-from-right-4 duration-500">
              <h2 className="text-3xl font-black mb-1 uppercase tracking-tighter text-white">{questions[formStep].label}</h2>
              <p className="text-[10px] text-slate-500 mb-10 font-bold uppercase tracking-widest">{questions[formStep].desc}</p>
              <input ref={inputRef} name={questions[formStep].name} value={(formData as any)[questions[formStep].name]} onChange={handleChange} onKeyDown={handleKeyDown} placeholder={questions[formStep].placeholder} className="w-full bg-transparent border-b border-white/10 focus:border-lime-400 py-4 text-2xl text-white focus:outline-none transition-all placeholder:text-white/5 uppercase font-bold tracking-tight" />
              {error && <p className="text-red-500 text-[9px] font-black uppercase mt-4 tracking-widest">{error}</p>}
              <div className="flex flex-wrap gap-4 mt-12">
                <button onClick={nextFormStep} className="px-10 py-5 bg-lime-400 text-black font-black uppercase text-xs tracking-widest hover:bg-white transition-all rounded-[7px]">{formStep === 5 ? 'Analyze Brand' : 'Next'}</button>
                <button onClick={handleLoadDemo} className="px-10 py-5 border border-white/10 text-white font-black uppercase text-xs tracking-widest hover:bg-white/5 transition-all rounded-[7px]">Load Demo</button>
              </div>
            </div>
          </div>
        )}
        ...
        {step === 'SCANNING' && (
          <div className="flex flex-col items-center justify-center p-20 text-center relative z-10 min-h-[600px]">
            ...
            <p className="text-lime-400/60 text-[10px] font-black uppercase tracking-[0.4em] mb-12 animate-pulse">{loadingProgress < 50 ? "Crawling Public Records..." : "Analyzing AI Sentiments..."}</p>
        ...
        {step === 'GATE' && (
          <div className="grid md:grid-cols-2 animate-in fade-in slide-in-from-bottom-6 duration-1000 relative z-10 min-h-[600px]">
            <div className="p-20 border-r border-white/5 bg-slate-900/10">
              ...
              <h2 className="text-6xl font-black mb-8 uppercase tracking-tighter text-white leading-[0.85]">Results <br/>Ready.</h2>
              <p className="text-slate-500 text-xs font-bold uppercase tracking-widest leading-relaxed max-w-sm">The visibility report for <span className="text-white font-black">{formData.brandName}</span> is now available for review.</p>
            </div>
            <div className="p-20 flex flex-col justify-center bg-white/[0.01]">
              ...
              <form onSubmit={handleSignup} className="space-y-6">
                <input type="email" value={userEmail} onChange={(e) => setUserEmail(e.target.value)} placeholder="BUSINESS EMAIL" className="w-full bg-black/40 border border-white/10 px-8 py-6 text-white focus:outline-none focus:border-lime-400 transition-all text-[11px] font-black uppercase tracking-widest rounded-[7px]" required />
                <button type="submit" className="w-full py-6 bg-lime-400 text-black font-black uppercase text-[11px] tracking-[0.4em] hover:bg-white transition-all rounded-[7px]">Access Report</button>
              </form>
            </div>
          </div>
        )}
        ...
        {step === 'COMPLETE' && result && (
          <div className="flex flex-col md:flex-row min-h-[800px] animate-in fade-in duration-1000">
            ...
            <div ref={reportRef} className="flex-1 p-0 md:p-8 lg:p-12 overflow-y-auto max-h-[800px] relative bg-slate-950 report-content flex flex-col">
                ...
                <div className="absolute inset-0 z-[100] flex items-center justify-center p-12 text-center bg-slate-950/40 backdrop-blur-3xl animate-in fade-in duration-500">
                  <div className="max-w-md p-10 bg-slate-900/90 border border-white/10 rounded-[12px] shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden group">
                    ...
                    <div className="pt-8 border-t border-white/5">
                      <button onClick={() => setActivePage('SUMMARY')} className="text-[10px] text-slate-500 hover:text-white font-black uppercase tracking-[0.4em] transition-all flex items-center justify-center gap-2 mx-auto">
                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M10 19l-7-7m0 0l7-7m-7 7h18" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                        Back to Summary
                      </button>
                    </div>
                  </div>
              ...
              <div className={`flex-1 flex flex-col transition-all duration-1000 ${isRestricted(activePage) ? 'blur-xl pointer-events-none select-none opacity-40 grayscale-[0.8]' : 'opacity-100'}`}>
                {activePage === 'COVER' && (
                  <div className="flex-1 flex flex-col justify-center items-center text-center p-20 animate-in fade-in zoom-in-95 duration-1000 relative z-10 h-full min-h-[700px]">
                    ...
                    <div className="mb-20 relative">
                      ...
                      <h1 className="text-8xl md:text-10xl font-black uppercase tracking-tighter text-white mb-6 leading-[0.75] animate-in zoom-in-95 duration-1000">AIEO <br/><span className="text-slate-800">AUDIT</span></h1>
                    ...
                    <div className="mt-auto w-full max-w-2xl border-t border-white/5 pt-12 text-left">
                      <div className="grid grid-cols-2 gap-16">
                        ...
                        <div className="space-y-4">
                          ...
                          <h3 className="text-2xl font-black text-white uppercase tracking-tight">{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</h3>
                          <div className="flex items-center gap-2">
                  ...
                  <div className="p-4 md:p-0 animate-in fade-in duration-500 relative z-10 flex flex-col h-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 mb-16 items-start">
                      <div className="p-10 bg-white/[0.02] border border-white/5 border-l-[12px] border-l-lime-400 rounded-[8px] shadow-2xl relative overflow-visible min-h-[280px] flex flex-col justify-center h-auto backdrop-blur-md">
                        <div className="absolute -top-4 -right-4 w-12 h-12 bg-slate-900 border border-white/10 flex items-center justify-center rounded-full text-lime-400 font-black italic">!</div>
                      ...
                      <div className="grid grid-cols-2 gap-4 lg:gap-6">
                        {[
                          { label: 'Overall Visibility', val: `${result.overallScore}%`, color: result.overallScore < 50 ? 'text-red-500' : 'text-lime-400', desc: 'Weighted Aggregate' },
                    ...
                    <div className="mt-auto pt-12 border-t border-white/5">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {result.sources?.length ? result.sources.map((source, i) => (
                          <a key={i} href={source.uri} target="_blank" rel="noopener noreferrer" className="p-6 bg-slate-900/60 border border-white/5 rounded-[8px] hover:border-lime-400/30 transition-all flex items-center gap-6 group">
                            <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center shrink-0 text-[10px] font-black group-hover:text-lime-400">{i+1}</div>
                          ...
                          <div className="col-span-full p-8 bg-white/[0.01] border border-dashed border-white/10 rounded-[8px] text-center">
                            <p className="text-[11px] text-slate-600 font-black uppercase tracking-widest italic">No external citations extracted for this specific entity footprint.</p>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12 items-stretch">
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                              ...
                              <PolarAngleAxis 
                                ...
                                tick={{ fill: '#64748b', fontSize: 8, fontWeight: 900, textTransform: 'uppercase' }} 
                              />
                      ...
                      <div className="p-10 bg-white/[0.02] border border-white/5 rounded-[12px] shadow-2xl flex flex-col backdrop-blur-sm">
                        ...
                        <div className="space-y-10 flex-1 flex flex-col justify-center">
                          {[
                            { label: 'Hallucination Risk', value: result.hallucinationRisk, color: 'text-amber-500', bar: 'bg-amber-500', icon: 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
                            { label: 'Logic Integrity', value: result.logicIntegrity, color: 'text-emerald-500', bar: 'bg-emerald-500', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
                            { label: 'Model Alignment', value: result.foundation?.score || 0, color: 'text-white', bar: 'bg-white', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
                            { label: 'Extraction Speed', value: result.aiVisibilityIndex || 0, color: 'text-lime-400', bar: 'bg-lime-400', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' }
                          ].map((m, i) => (
                            <div key={i} className="relative">
                              <div className="flex justify-between items-center mb-4">
                                <div className="flex items-center gap-3">
                                  <svg className={`w-3.5 h-3.5 ${m.color}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d={m.icon}/></svg>
                        ...
                        <div className="space-y-8 flex-1 flex flex-col justify-center">
                          {(result.aiBreakdown || []).map((engine, i) => (
                            <div key={i} className="p-6 bg-slate-900/40 border border-white/5 rounded-[8px] relative overflow-hidden group hover:border-lime-400/20 transition-all">
                              ...
                              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed line-clamp-2 italic">
                                "{engine.keyTakeaway}"
                              </p>
                            </div>
                          ))}
                  ...
                  <div className="p-4 md:p-6 lg:p-10 animate-in fade-in duration-500 relative z-10 flex flex-col">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pb-16">
                      {(result.technicalChecklist || []).map((item, i) => (
                        <div key={i} className={`group p-6 bg-white/[0.01] border border-white/5 rounded-[8px] border-l-4 transition-all hover:bg-white/[0.03] hover:-translate-y-1 ${item.status === 'Pass' ? 'border-l-emerald-500/50 hover:border-l-emerald-400' : item.status === 'Warning' ? 'border-l-amber-500/50 hover:border-l-amber-400' : 'border-l-red-500/50 hover:border-l-red-500'}`}>
                          <div className="flex justify-between items-start gap-4 mb-4">
                            <div className="flex flex-col">
                              ...
                              <span className="text-[11px] text-slate-200 font-black uppercase tracking-widest leading-tight group-hover:text-white transition-colors">{item.point}</span>
                            </div>
                            <span className={`text-[9px] font-black uppercase px-2 py-1 rounded shrink-0 shadow-sm ${item.status === 'Pass' ? 'text-emerald-400 bg-emerald-400/5 border border-emerald-400/20' : item.status === 'Warning' ? 'text-amber-400 bg-amber-400/5 border border-amber-400/20' : 'text-red-500 bg-red-500/5 border border-red-500/20'}`}>{item.status}</span>
                          </div>
                          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight leading-relaxed line-clamp-2">
                            {item.observation}
                          </p>
                        </div>
                      ))}
                  ...
                  <div className="p-4 md:p-6 lg:p-10 animate-in fade-in duration-500 relative z-10 flex flex-col h-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-16">
                      {(result.socialFootprint || []).map((platform, i) => (
                        <div key={i} className="p-10 bg-white/[0.02] border border-white/5 rounded-[12px] flex flex-col gap-6 relative group hover:border-white/10 transition-all backdrop-blur-sm">
                          <div className="flex justify-between items-center gap-6">
                            <h4 className="text-2xl font-black text-white uppercase tracking-tighter truncate">{platform.platform}</h4>
                            <div className="flex flex-col items-end shrink-0">
                               <span className={`px-2 py-0.5 text-[8px] font-black uppercase rounded-[4px] mb-1 ${platform.presence === 'High' ? 'bg-lime-400 text-black' : platform.presence === 'Medium' ? 'bg-white/10 text-white' : 'bg-slate-800 text-slate-500'}`}>{platform.presence} Signal</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-4 py-4 border-y border-white/5">
                             <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest shrink-0">Analysis:</span>
                             <span className={`text-[11px] font-black uppercase truncate ${platform.sentiment.toLowerCase().includes('positive') ? 'text-emerald-400' : platform.sentiment.toLowerCase().includes('negative') ? 'text-red-400' : 'text-white'}`}>{platform.sentiment}</span>
                          </div>
                          <p className="text-slate-400 text-[12px] font-bold uppercase tracking-widest leading-relaxed line-clamp-4 min-h-[96px]">
                  ...
                  <div className="p-4 md:p-6 lg:p-10 animate-in fade-in duration-500 relative z-10 flex flex-col">
                    ...
                    <div className="space-y-8 pb-16">
                      {(result.pillarAnalysis || []).map((pillar, i) => (
                        <div key={i} className="p-10 bg-white/[0.02] border border-white/5 rounded-[12px] flex flex-col md:flex-row gap-12 items-center group hover:bg-white/[0.04] transition-all">
                          <div className="w-20 h-20 bg-slate-900 border-2 border-white/10 flex items-center justify-center font-black text-3xl text-lime-400 rounded-[12px] shrink-0 shadow-[0_10px_30px_rgba(0,0,0,0.3)]">0{i+1}</div>
                  ...
                  <div className="p-4 md:p-6 lg:p-10 animate-in fade-in duration-500 relative z-10 flex flex-col">
                    ...
                    <div className="grid grid-cols-1 gap-10 pb-16">
                      {(result.globalRecommendations || []).map((rec, i) => (
                        <div key={i} className="p-12 bg-slate-900/60 border border-white/10 rounded-[12px] relative overflow-hidden group hover:border-lime-400/30 transition-all backdrop-blur-xl">
                          ...
                          <div className="flex flex-wrap items-center gap-6 mb-8 relative z-10">
                             <div className={`px-6 py-2 text-[10px] font-black uppercase rounded-[6px] shadow-lg ${rec.priority.toLowerCase() === 'high' ? 'bg-red-600 text-white' : 'bg-lime-400 text-black'}`}>{rec.priority} Priority Action</div>
                             <div className="flex items-center gap-2">
                               <svg className="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                          ...
                          <p className="text-slate-400 text-base font-bold uppercase tracking-widest leading-relaxed max-w-3xl relative z-10 italic">
                            {rec.action}
                          </p>
